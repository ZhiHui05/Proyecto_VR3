...
class VirtualDroneApplication:
    ...
    def __init__(
        ...
        self._enable_3d = enable_3d and self._cfg.get(
            "application.visualization_3d", True
        )
        ...
        if not self._thread_manager.initialize(
            ...
            enable_visualization=self._enable_3d,
        ):
        ...
        self._target_fps = self._cfg.get("camera.fps", 30)
        self._camera_fps = 0.0
        self._vis_fps = 0.0
        self._loop_count = 0
        self._loop_timer = time.perf_counter()
        self._fps_log_timer = time.perf_counter()
    ...
    def run(self) -> None:
        ...
        target_interval = 1.0 / max(self._target_fps, 1)
        try:
            while self._running:
                ...
                if now - self._loop_timer >= 1.0:
                    ...
                    if now - self._fps_log_timer >= 5.0:
                        self._logger.info(
                            f"Display FPS: {self._camera_fps:.1f}, "
                            f"new frames: {self._new_frame_count}"
                        )
                        self._new_frame_count = 0
                        self._fps_log_timer = now

                errors = self._thread_manager.state.consume_errors()
                if errors:
                    self._logger.warning(
                        f"Component errors: {errors[-3:]}"
                    )

                frame, frame_time = self._thread_manager.state.get_latest_frame_with_timestamp()
...
Examples:
  ...
  python -m src.main --config configs/ --headless
    ...
    parser.add_argument(
        "--no-3d",
        action="store_true",
        help="Disable the Open3D 3D visualizer",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
