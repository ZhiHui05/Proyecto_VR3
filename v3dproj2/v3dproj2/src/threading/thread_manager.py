...
class SharedState:
    ...
    def consume_errors(self) -> list[str]:
        with self._lock:
            errors = list(self._component_errors)
            self._component_errors.clear()
            return errors

    def set_latest_frame(self, frame: Optional[np.ndarray], timestamp: float = 0.0) -> None:
        with self._lock:
    ...
    def get_latest_frame_timestamp(self) -> float:
        with self._lock:
            return self._latest_frame_time

    def get_latest_frame_with_timestamp(self) -> Tuple[Optional[np.ndarray], float]:
        with self._lock:
                ...
                if _O3D_IMPORT_OK:
                    ...
                    if not self._visualizer.initialize():
                        self._logger.error(
                            ...
                            "continuing without 3D visualization"
                        )
        ...
        def camera_loop() -> None:
            ...
            while not self._state.stop_event.is_set():
                try:
                    ...
                    if timestamp == last_forwarded_time:
                        # No new frame from camera yet; avoid flooding shared state
                        time.sleep(0.001)
                        continue
                    last_forwarded_time = timestamp
                    try:
                        self._state.frame_queue.put_nowait((frame, timestamp))
                    except Exception:
                        pass
                    self._state.set_latest_frame(frame, timestamp)
                    forwarded += 1
                    now = time.perf_counter()
                    if now - log_timer >= 5.0:
                        self._logger.info(
                            f"Camera loop forwarded {forwarded} frames in last 5s"
                        )
                        forwarded = 0
                        log_timer = now
                except Exception as e:
        ...
        def tracking_loop() -> None:
            ...
            while not self._state.stop_event.is_set():
                try:
                    ...
                    # Drain queue to always process the most recent frame
                    while not self._state.frame_queue.empty():
                        try:
                            frame, _timestamp = self._state.frame_queue.get_nowait()
                        except Empty:
                            break
